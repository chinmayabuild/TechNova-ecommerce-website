  ...
  return (
    <div className="relative flex flex-col items-center  lg:h-[85vh] sm:h-[38vh] mt-2 overflow-hidden bg-[#D9190E] mx-auto w-[95vw] justify-center">
      ...
      <button
        id="next"
        onClick={goToNext}
        className="absolute right-2 sm:right-4 md:right-10 lg:right-20 top-1/2 transform -translate-y-1/2 text-xl sm:text-2xl md:text-3xl lg:text-5xl rounded-full w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 flex items-center justify-center text-white  hover:text-[#e9660d] z-20"
      >
        &gt;
      </button>

      {/* View Product Button */}
      <Button className="mt-2 sm:mt-4 rounded px-3 py-1 sm:px-6 sm:py-2 bg-black hover:bg-white hover:text-black text-white text-xs sm:text-base">
        View Product
      </Button>
    </div>
  );
