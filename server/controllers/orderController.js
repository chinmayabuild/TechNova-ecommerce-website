...
const getMetrics = async (req, res) => {
  ...
  try {
    ...
    const end = Date(endDate || new Date());
    ...
    // Calculate the last month orders
    ...
    // Calculate the total amount of last months orders
    ...
    const lastHourOrders = await Order.find({
      createdAt: { $gte: lastHour, $lt: new Date() },
    });
    ...
    const lastHourGrowth = previousDayOrders.length
      ? (lastHourOrders.length / previousDayOrders.length) *
        previousDayOrders.length *
        100
      : 0;
    ...
    // Products delivered in last 6 months
    ...
    // Get the month wise for eg :{jan: {keybord: 1, mouse:2, headset:1}}
